...
class DistributorTokenView(IntegratedAuthMixin, APIView):
    def get(self, request):
        try:
            distributor = get_distributor(self.request)
            queryset = DistributorToken.objects.filter(revoked=False, distributor=distributor)
            serializer = DistributorTokenSerializer(queryset)
            
            return Response({
                'message': "Tokens",
                "Tokens dict": serializer.data
            }, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({
